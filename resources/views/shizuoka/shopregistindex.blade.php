		...
		<div class="row justify-content-center">
			<div class="col-md-10">
				<div class="card mt-3 mb-3">
					<div class="card-header text-center lead">あなたの知っているお店情報を入力してください。</div>
					<div class="card-body">
						<form method="POST" action="{{ route('shop_regist') }}" enctype="multipart/form-data" novalidate>

							{{--  お店の名前 --}}
							<div class="form-group row">
								<label for="shop_name" class="col-md-3 col-form-label text-md-center">お店の名前</label>
								<div class="col-md-9">
									{{-- @invalid定義先：AppServiceProvider.php --}}
									<input id="" type="text" class="form-control @invalid('shop_name')" name="shop_name" value="{{ old('shop_name') }}" required autocomplete="shop_name" autofocus>
									...
									<div class="col-md-9">
										<div>
											@foreach (session('genre_data') as $genre_cd => $genre_name)
												<input id="{{ $genre_name }}" type="radio" name="genre_cd" value="{{ $genre_cd }}" {{ old('genre_cd', 1) == $genre_cd ? "checked" : '' }}>
								...
								<div class="col-md-3">
									<select name="city_cd" id="city_cd" class="form-control">
										@foreach (session('city_data') as $city_cd => $city_name)
											<option value="{{ $city_cd }}" {{ old('city_cd', 1) == $city_cd ? "selected" : '' }}>{{ $city_name }}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group row">
								...
								<div class="col-md-9">
									<textarea class="form-control @invalid('shop_detail')" name="shop_detail" id="shop_detail" cols="30" rows="10" required>{{ old('shop_detail') }}</textarea>
